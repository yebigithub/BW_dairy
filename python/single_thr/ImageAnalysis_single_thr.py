...
def threshold_selecting(hue):
  ...
  '''
  for thresh in range(int(np.mean(hue)), int(np.mean(hue))+1):
    ...
    if (y < 0 or y+hh > 255 or x < 0): 
           print('Threshold for this image doesnt work \n')
           qc = False
           return qc, fill_img
      ...
      with open(summ, "w", newline = "") as output:
          ...
          for root, dirs, files in os.walk(depthdir):
            ...
            for file in files:
                ...
                qc, fill_img = threshold_selecting(hue=hue)
